# Copyright 2014 Vitor Augusto Ferreira Santa Rita

# Decoding may be employed in two different ways:
# 1) by local resolution, passing the encoded lat,lon pair and the decoder gps position to the function cpr_resolve_local();
# 2) or by global resolution, that needs two encoded values: one of odd type and another of even type; with that, the function
# to call is cpr_resolve_global(). One import need is that the values must have been acquired within an interval time smaller than
# 10 seconds.
...
def calc_nl_table():
	...
	if nl_table is None:
		...
		for declat_in in lat_boundaries[1:]:
			for j in range(declat_in,0,-1): # inefficient but is done only once
				if j >= 87 or j == 0: # do not calculate these as they are obtained at nl()
					continue
